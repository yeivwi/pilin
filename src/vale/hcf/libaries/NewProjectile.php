...
abstract class NewProjectile extends \pocketmine\entity\projectile\Projectile {
    ...
    public function move(float $dx, float $dy, float $dz) : void {
		...
		foreach($this->level->getCollidingEntities($this->boundingBox->addCoord($newDiff->x, $newDiff->y, $newDiff->z)->expand(0.3, 0.3, 0.3), $this) as $entity){
			if($entity->getId() === $this->getOwningEntityId() and $this->ticksLived < 5){
				continue;
			}

			$entityBB = $entity->boundingBox->expandedCopy(0.01, 0.01, 0.01);
